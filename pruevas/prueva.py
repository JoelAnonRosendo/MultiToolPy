...
class SystemMonitor:
    def __init__(self, root):
        self.root = root
        self.root.title("Aurelius Multitool")
    ...
    def create_widgets(self):
        ...
        # Crear frame del menú lateral
        menu_frame = ttk.Frame(main_frame, width=200)
        menu_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))

        # Crear frame del contenido
        content_frame = ttk.Frame(main_frame)
        content_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # Menú lateral
        menu_items = ["Home Page", "Troubleshooting", "Windows Style", "Package Manager", 
                      "Multimedia", "Crypto", "Networking", "Gaming", "Macro", "Miscellaneous"]
        for item in menu_items:
            ttk.Button(menu_frame, text=item).pack(fill=tk.X, pady=2)
